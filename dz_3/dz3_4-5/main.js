...
function initEvenHandles() { //обработчик событий init
...
function addButtonClicked() {
  ...
  // 1.2.1. Если выпадающий список еще не содержит добавляемое значение, последовательно вызывает функции addColor, chooseColor и setColor, передавая им в качестве входного параметра значение, которое пользователь ввел в текстовое поле.
    ...
    if (checkIfColorAdded(value) === false) {
      addColor(value);
      chooseColor(value);
      setColor(value);
    }
...
function removeSelectedColorClicked() {
  ...
  let valueGSV = getSelectedValue();
  if (valueGSV === 'not selected') {
    alert("'Please choose a value to remove'")
  }
  //if (getSelectedValue() !== 'not selected') 
  else {
    removeColor(valueGSV);
    reset();
  }
...
function getHTMLElement(selector) {
  let element = document.querySelector(selector);// 1. находит HTML элемент по селектору
  return element;// 2. возвращает найденный HTML элемент
...
function checkIfColorAdded(color) {
  ...
  for (let i = 1; i < htmlElements.selectColor.length; i++) {//цикл для пробежки по массиву
    //colorAdded = true; лишнее
    if (htmlElements.selectColor[i].value === color) {//условие что если htmlElements.selectColor содержит передваемы цвет colorAdded присваевает значение true
      colorAdded = true;
    }
  }
  return colorAdded;
...
function chooseColor(color) {
  ...
  for (let i = 1; i < htmlElements.selectColor.length; i++) {
    ...
    if (htmlElements.selectColor[i].value === color) {
      setSelectedIndex(i);//передаем в setSelectedIndex  
    }
  }
...
function setSelectedIndex(idx) {
  htmlElements.selectColor.selectedIndex = idx;//index номер опции в списке селекта
}
...
function checkIfColorCanBeAdded(color) {
  if (color.indexOf('rgb') > -1 || color === '') {//indexOf=возвращает номер элемента или -1 если его нет.
    return false;
  }
